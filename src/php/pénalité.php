...
if (!isset($_SESSION['eleves'])) {
  $_SESSION['eleves'] = [
    ['id' => 1, 'nom' => 'Dupont', 'prenom' => 'Alice', 'retard' => 3],
    ['id' => 2, 'nom' => 'Martin', 'prenom' => 'Bob', 'retard' => 1],
    ['id' => 3, 'nom' => 'Durand', 'prenom' => 'Chloé', 'retard' => 2],
  ];
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['modifier'])) {
  ...
  foreach ($eleves as &$eleve) {
    if ($eleve['id'] === $id_modif) {
      ...
      $eleve['retard'] = (float)$_POST['retard'];
      break;
    }
  }
  unset($eleve);
...
<form method="post" class="mb-3 d-flex gap-2 align-items-end">
  ...
  <input type="number" name="retard" placeholder="retard" min="0" max="20" step="0.1" required class="form-control" style="width:180px;">
    ...
    <tr>
      <?php if ($edit_id === $eleve['id']): ?>
        ...
        <form method="post" style="display:contents;">
          ...
          <td>
            <input type="number" name="retard" value="<?= $eleve['retard'] ?>" min="0" max="20" step="0.1" class="form-control" required>
          </td>
          <td>
      ...
      <?php else: ?>
        ...
        <td><?= isset($eleve['retard']) ? $eleve['retard'] : '' ?></td>
        <td>
